...
// app.use(cors({
//     origin: ["http://localhost:3000", "http://192.168.33.78:3000", "http://192.168.1.145:3000", "*"],
...
app.post("/get/posted/count/questions", async (req, res) =>{
    const {user, img, Questio, a, b, c, d, Ans, lang} = req.body;

    try{
        const Qno_length = await QuestionModule.find({lang})
        const hash = Ans.trim().toLowerCase();
        await QuestionModule.create({Time, user, img, Questio, qno : Qno_length.length+1, a, b, c, d, Ans : hash, lang})
